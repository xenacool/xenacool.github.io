...
RunSlice ==
    ...
    (* When the slice budget is exhausted, the request RESOLVES (not stalls):
     * the runtime advances its cursor at step_pg_rpg_simulation in
     * runtime/src/game_loop_helpers.rs (the Progress branch settling as
     * `PgRpgSimulationSlice`), so at the last slice (workRemaining = 1) the
     * cursor (workerProcessed) advances to requestSeq and the next slice can
     * continue from where the simulation left off — the worker does NOT spin
     * at "Simulating ... progress N".  appliedSeq (history committed to the
     * browser) stays at requestSeq - 1 until the boundary (Ready / Completed)
     * fires, in ApplyResponse, where appliedSeq catches up to requestSeq. *)
    /\ workerProcessed' = (IF workRemaining = 1 THEN requestSeq ELSE workerProcessed)
