...
// Hard heartbeat deadline: the maximum number of simulation slices a single
// simulation may consume before the worker stops re-enqueuing instead of
// spinning (see spec/AnimationAcc.tla's RunSimulationSlice, which caps
// simulationSteps at MaxSteps).  Each slice is already bounded by the
// runtime's BOUNDARY_WORK_BUDGET (MaxSimulationWork); this caps the total
// work a stalled simulation can burn.  The value is generous relative to a
// real game's slice count but finite, so a simulation that never reaches a
// boundary terminates rather than spinning forever.
const MAX_SIMULATION_STEPS: u32 = 1024;

/// True once the simulation has consumed its slice budget the maximum number
/// of times (simulationSteps >= MaxSteps).  Maps to spec/AnimationAcc.tla's
/// RunSimulationSlice, where simulationSteps is no longer incremented once it
/// reaches MaxSteps and the simulation stops rather than continuing to spin.
pub(crate) fn simulation_slice_cap_hit(simulation_steps: u32, max_steps: u32) -> bool {
...
impl UnifiedWorker {
    ...
    fn auto_step_if_ready(&mut self, cx: &mut Context<'_>) {
        ...
        if !self.is_simulating
            ...
            || simulation_slice_cap_hit(self.simulation_steps, MAX_SIMULATION_STEPS)
    ...
    fn finish_auto_step(
        ...
        if let RuntimeResponse::PgRpgSimulationSlice { work_units, .. } = &res {
            // A slice budget was exhausted without reaching a boundary: the
            // request resolved here (see spec/SimulationBridge.tla's RunSlice)
            // but the game is still advancing, so re-enqueue a fresh auto-step
            // to continue toward the next boundary or completion.  This
            // replaces the old `SimulationProgress` loop that re-submitted the
            // same request and stalled the worker at `progress 8` forever.  The
            // game either reaches a boundary (or completes) after a bounded
            // number of requests, or the fixture leaves the boundary
            // unreachable — in which case the worker stops rather than spinning
            // without progress.
            self.push_debug_trace(format!(
                "slice budget exhausted without a boundary; continuing auto step (work_units={work_units})"
            ));
            self.push_output(WorkerOutput::RuntimeResponse(Box::new(
                RuntimeResponse::PgRpgSimulationSlice {
                    work_units: *work_units,
                    history: HistoryManager::new(),
                },
            )));
            // Track how many slices this simulation has consumed (see
            // spec/AnimationAcc.tla's RunSimulationSlice: simulationSteps <
            // MaxSteps).  Once the slice budget is exhausted the maximum
            // number of slices the worker stops re-enqueuing rather than
            // spinning (the hard heartbeat deadline).
            self.simulation_steps = self.simulation_steps.saturating_add(1);
            if simulation_slice_cap_hit(self.simulation_steps, MAX_SIMULATION_STEPS) {
                self.terminate_after_slice_budget(format!(
                    "simulation exceeded {MAX_SIMULATION_STEPS} slices without reaching a boundary"
                ), cx);
                return;
            }
    ...
    fn terminate_after_slice_budget(&mut self, message: String, cx: &mut Context<'_>) {
        // Stop re-enqueuing and terminate cleanly, matching the model's
        // RunSimulationSlice once simulationSteps reaches MaxSteps (phase
        // returns to Ready; no more slicing).  Emit an honest terminal
        // runtime error rather than stranding the UI in an unbounded
        // Simulating stall.
        ...
        self.transient_state.facing_pending = false;
        self.transient_state.input_enabled = false;
        self.transient_state.action_feedback = Some(message.clone());
        self.push_debug_trace(format!("simulation exceeded budget: {message}"));
        self.push_output(WorkerOutput::RuntimeResponse(Box::new(
