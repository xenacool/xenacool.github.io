    ...
    pub(crate) fn handle_simulation_response(
        ...
        match pending {
            ...
            PendingSimulation::AnimationAck {
                ...
                // ACK responses can carry presentation cleanup (for example,
                ...
                if matches!(
                    ...
                    self.transient_state.wait_pending = false;
                    self.transient_state.facing_pending = false;
                    self.transient_state.facing_pending = true;
                    self.transient_state.input_enabled = true;
                } else if wait {
                    self.transient_state.wait_pending = false;
                    self.transient_state.facing_pending = false;
                    ...
                    let unit_id = self.transient_state.active_unit_id.unwrap_or_default();
                    self.enqueue_simulation_request(
                        RuntimeRequest::RefreshAvailableActions { unit_id },
                        PendingSimulation::ActionsRefresh,
                    );
                }
                self.push_output(WorkerOutput::TransientState(Box::new(
                    self.transient_state.clone(),
            ...
            PendingSimulation::ActionsRefresh => {
                if let RuntimeResponse::AvailableActionsRefreshed { actions } = &response {
                    self.current_actions = Some(actions.clone());
                    self.transient_state.available_actions = Some(actions.clone());
                    self.transient_state.active_unit_id = Some(actions.unit_id);
                    // Replace costs and affordability authoritatively without
                    // discarding the menu branch that initiated the action.
                    // The renderer can then restore focus to that ability.
                    self.transient_state.ability_targets = None;
                    self.transient_state.input_enabled = true;
                    self.is_simulating = false;
                    self.push_debug_trace(format!(
                        "unified worker refreshed authoritative actions for unit {} after animation ack",
                        actions.unit_id
                    ));
                    self.push_output(WorkerOutput::TransientState(Box::new(
                        self.transient_state.clone(),
