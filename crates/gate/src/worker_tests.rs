...
proptest::proptest! {
    #[test]
    fn simulation_slice_budget_is_capped_after_max_slices(
        max_steps in 1u32..1024,
        steps in 0u32..1024,
    ) {
        // The cap is not hit until steps reaches max_steps, then stays hit
        // (saturating) — matching spec/AnimationAcc.tla's RunSimulationSlice,
        // where simulationSteps no longer increments once it reaches MaxSteps.
        prop_assert_eq!(
            simulation_slice_cap_hit(steps, max_steps),
            steps >= max_steps,
        );
        // Once the cap is hit it stays hit under saturating growth.
        prop_assert_eq!(simulation_slice_cap_hit(u32::MAX, max_steps), true);
    }
}

#[test]
fn a_simulation_below_the_cap_is_not_stopped() {
    assert!(!simulation_slice_cap_hit(0, MAX_SIMULATION_STEPS));
    assert!(!simulation_slice_cap_hit(
    ...
    // Slicing beyond the cap still reports it as hit (saturating).
    assert!(simulation_slice_cap_hit(
        MAX_SIMULATION_STEPS + 1,
        MAX_SIMULATION_STEPS
    ));
