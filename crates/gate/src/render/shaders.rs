...
uniform bool uSpritestackCutout;
...
// Keep this synchronized with the import-time RGBA cutoff. Processed
// spritestack pixels are either fully transparent or fully opaque.
const float SPRITESTACK_ALPHA_CUTOFF = 128.0 / 255.0;
...
void main() {
    ...
    if (uUseTexture) {
        ...
        if (uSpritestackCutout && texColor.a < SPRITESTACK_ALPHA_CUTOFF) discard;
        vec3 linearTexColor = pow(texColor.rgb, vec3(2.2));
        color = mix(color, linearTexColor, texColor.a);
        if (uSpritestackCutout) {
            // Spritestacks use cutout semantics, not translucent blending.
            alpha = 1.0;
        } else {
            alpha = texColor.a;
        }
