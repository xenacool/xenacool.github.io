...
use pystral_core::render::{ERROR_MODE_ENABLED, RenderError};
...
pub fn interpolate_property(from: &PropertyValue, to: &PropertyValue, t: f32) -> PropertyValue {
    match (from, to) {
        (PropertyValue::Float(f1), PropertyValue::Float(f2)) => {
            PropertyValue::Float(f1 + (f2 - f1) * t)
        }
        (PropertyValue::Vec3(v1), PropertyValue::Vec3(v2)) => {
            PropertyValue::Vec3(*v1 + (*v2 - *v1) * t)
        }
...
impl EntityExt for EntityState {
    fn get_float(&self, key: &str, default: f32) -> RenderResult<f32> {
        if let Some(PropertyValue::Float(v)) = self.properties.get(key) {
            if ERROR_MODE_ENABLED.load(Ordering::Relaxed) {
                Err(RenderError::new(
                    format!("Property {} found but ERROR_MODE_ENABLED is set", key),
                    *v,
                ))
            } else {
                Ok(*v)
        ...
        if let Some(prop) = self.properties.get("material") {
            match prop {
                PropertyValue::Material(m) => {
                    if ERROR_MODE_ENABLED.load(Ordering::Relaxed) {
                        Err(RenderError::new(
                            "Property material found but ERROR_MODE_ENABLED is set",
                            m.clone(),
                        ))
                    } else {
                        Ok(m.clone())
                ...
                PropertyValue::String(name) => {
                    if let Some(m) = materials.get(name) {
                        if ERROR_MODE_ENABLED.load(Ordering::Relaxed) {
                            Err(RenderError::new(
                                format!("Material {} found but ERROR_MODE_ENABLED is set", name),
                                m.clone(),
                            ))
                        } else {
                            Ok(m.clone())
                        ...
                        Err(RenderError::new(
                            format!("Material {} not found in world", name),
                            default,
                        ))
                    }
                }
                _ => Err(RenderError::new(
                    "Property material is not a Material or String",
                    default,
                )),
            }
        } else {
    ...
    fn get_hex_map(&self) -> RenderResult<pystral_core::domain::HexMap> {
        ...
        if let Some(PropertyValue::HexMap(m)) = self.properties.get(key) {
            if ERROR_MODE_ENABLED.load(Ordering::Relaxed) {
                Err(RenderError::new(
                    "Property map found but ERROR_MODE_ENABLED is set",
                    m.clone(),
                ))
            } else {
                Ok(m.clone())
            ...
            Err(RenderError::new(
                ...
                pystral_core::domain::HexMap::new(),
            ))
        }
    }
    ...
    fn get_lighting(&self) -> RenderResult<pystral_core::domain::LightingConfig> {
        ...
        if let Some(PropertyValue::Lighting(l)) = self.properties.get(key) {
            if ERROR_MODE_ENABLED.load(Ordering::Relaxed) {
                Err(RenderError::new(
                    "Property lighting found but ERROR_MODE_ENABLED is set",
                    l.clone(),
                ))
            } else {
                Ok(l.clone())
    ...
    fn get_collision(&self) -> RenderResult<Option<pystral_core::domain::Shape3D>> {
        ...
        if let Some(PropertyValue::Shape3D(s)) = self.properties.get(key) {
            if ERROR_MODE_ENABLED.load(Ordering::Relaxed) {
                Err(RenderError::new(
                    "Property collision found but ERROR_MODE_ENABLED is set",
                    Some(s.clone()),
                ))
            } else {
                Ok(Some(s.clone()))
