...
pub fn render_commands_to_texture(
    gl: &GL,
        ...
        for cmd in call {
            match cmd {
                PainterCommand::MoveTo(x, y, _z) => {
                    let rx = if side == ViewSide::Mirrored {
                        width as f32 - x
                    } else {
                        x
                    };
                ...
                PainterCommand::QuadTo(x1, y1, _z1, x, y, _z) => {
                    let rx1 = if side == ViewSide::Mirrored {
                        width as f32 - x1
                    } else {
                        x1
                    };
                ...
                PainterCommand::CubicTo(x1, y1, _z1, x2, y2, _z2, x, y, _z) => {
                    ...
                    let rx2 = if side == ViewSide::Mirrored {
                        width as f32 - x2
                    } else {
                        x2
                    };
                ...
                PainterCommand::SetColor(front, mirrored) => {
                    let rgba = if side == ViewSide::Front {
                        front
                    } else {
                        mirrored
                    };
