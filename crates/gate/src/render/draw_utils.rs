...
pub fn set_model_matrix(gl: &GL, uniforms: &UniformLocations, matrix: &Mat4) {
    if let Some(loc) = uniforms.model.as_ref() {
...
pub fn set_material(
    ...
    if let Some(loc) = uniforms.obj_color.as_ref() {
        gl.uniform3f(Some(loc), color[0], color[1], color[2]);
    }
    if let Some(loc) = uniforms.roughness.as_ref() {
        gl.uniform1f(Some(loc), roughness);
    }
    if let Some(loc) = uniforms.metalness.as_ref() {
        gl.uniform1f(Some(loc), metalness);
    }
    if let Some(loc) = uniforms.emissive.as_ref() {
