...
pub(super) fn draw_spritestack(
    ctx: &mut RenderContext,
    worker_tx: &futures::channel::mpsc::UnboundedSender<crate::WorkerInput>,
    entity: &EntityState,
    state: &WorldState,
    sprite_pos: Vec3,
    _billboard_rot: glam::Mat4,
    entity_scale: f32,
    _cam_forward: Vec3,
    yaw: f32,
) {
    // Spritestacks are alpha-tested cutouts. Establish and restore the
    // explicit scene contract instead of inheriting blend/depth state from a
    // preceding material.
    ctx.gl.enable(GL::DEPTH_TEST);
    ctx.gl.depth_mask(true);
    ctx.gl.depth_func(GL::LEQUAL);
    ctx.gl.disable(GL::BLEND);
    ctx.gl
        .uniform1i(ctx.uniforms.spritestack_cutout.as_ref(), 1);

    if let Some(pystral_core::log::PropertyValue::String(asset_name)) =
        entity.properties.get("asset")
    {
        ...
        if let Some(textures) = ctx.spritestack_assets.get(&cache_key) {
            ...
            for (i, (color_tex, normal_tex)) in textures
                .color_textures
                .iter()
                .zip(textures.normal_textures.iter())
                .enumerate()
            {
                ...
                ctx.gl
                    .active_texture(web_sys::WebGlRenderingContext::TEXTURE0);
                ctx.gl
                    .bind_texture(web_sys::WebGlRenderingContext::TEXTURE_2D, Some(color_tex));
                ctx.gl.uniform1i(ctx.uniforms.texture.as_ref(), 0);

                ctx.gl
                    .active_texture(web_sys::WebGlRenderingContext::TEXTURE1);
                ctx.gl
                    .bind_texture(web_sys::WebGlRenderingContext::TEXTURE_2D, Some(normal_tex));
                ...
                ctx.sprite_mesh
                    .draw(&ctx.gl, ctx.attribs.pos, ctx.attribs.norm, ctx.attribs.uv);
            }
            ctx.gl.uniform1i(ctx.uniforms.use_normal_map.as_ref(), 0);
        }
    }

    // The scene's existing material contract enables blending outside this
    // cutout path.
    ctx.gl
        .uniform1i(ctx.uniforms.spritestack_cutout.as_ref(), 0);
        ...
        if let Some(collection) = ctx.asset_collection_cache.get(collection_name) {
            if let Some(stack) = collection.spritestacks.get(asset_name) {
                ...
                for slice in &stack.slices {
                    color_textures.push(create_texture(
                        &ctx.gl,
                        stack.width,
                        stack.height,
                        &slice.color_data,
                    ));
                    normal_textures.push(create_texture(
                        &ctx.gl,
                        stack.width,
                        stack.height,
                        &slice.normal_data,
                ...
                ctx.spritestack_assets.insert(
                    cache_key.to_string(),
                    crate::render::context::SpritestackTextures {
                        color_textures,
                        normal_textures,
                        width: stack.width,
                        height: stack.height,
...
pub(super) fn draw_collision(
    ...
    match shape {
        Shape3D::Capsule(capsule) => {
            let cyl_model = Mat4::from_translation(sprite_pos)
                * model_rot
                * Mat4::from_scale(Vec3::new(capsule.radius, capsule.radius, capsule.height));
            set_model_matrix(&ctx.gl, &ctx.uniforms, &cyl_model);
            ctx.cylinder_mesh.draw_wireframe(&ctx.gl, ctx.attribs.pos);

            let sphere_model_bottom =
                Mat4::from_translation(sprite_pos) * Mat4::from_scale(Vec3::splat(capsule.radius));
            set_model_matrix(&ctx.gl, &ctx.uniforms, &sphere_model_bottom);
            ctx.sphere_mesh.draw_wireframe(&ctx.gl, ctx.attribs.pos);

            let sphere_model_top = Mat4::from_translation(
                sprite_pos + model_rot.transform_vector3(Vec3::new(0.0, 0.0, capsule.height)),
            ) * Mat4::from_scale(Vec3::splat(capsule.radius));
            set_model_matrix(&ctx.gl, &ctx.uniforms, &sphere_model_top);
            ctx.sphere_mesh.draw_wireframe(&ctx.gl, ctx.attribs.pos);
        }
        Shape3D::Cube(size) => {
            let cube_model = Mat4::from_translation(sprite_pos)
                * model_rot
