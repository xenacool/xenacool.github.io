...
pub fn draw_scene(
    ...
    for entity in &state.entities {
        ...
        if let Some(map) = &current_map {
            // Every non-camera renderable is initialized through the shared
            // sprite property contract, which supplies a layer (including
            // render-only arrows and decorative rocks).
            let entity_layer = entity.get_float("layer", 0.0).log_fallback(worker_tx) as i32;
            for tile in &map.tiles {
                if tile.hex == current_hex && tile.layer == entity_layer {
    ...
    if transient.preview.is_some() {
        if let Some(preview_mesh) = ctx.unit_hex_mesh_cache.take() {
            draw_move_preview(
                ctx,
                worker_tx,
                state,
            ...
            ctx.unit_hex_mesh_cache = Some(preview_mesh);
        }
    }
}
...
fn draw_move_preview(
    ...
    hex_mesh: &Mesh,
    map: &Option<pystral_core::domain::HexMap>,
) {
    let Some(preview) = transient.preview.as_ref() else {
        return;
    };
    for destination in &preview.reachable {
        draw_preview_cell(
            ctx,
            layout,
            hex_mesh,
    ...
    if let Some(source) = &preview.source {
        draw_preview_cell(ctx, layout, hex_mesh, map, source, [0.95, 0.8, 0.15], 0.035);
    }
    if let Some(selected) = &preview.selected_destination {
        draw_preview_cell(
            ctx,
            layout,
            hex_mesh,
        ...
        if let Some(entity) = state
            .entities
            .iter()
            .find(|entity| entity.id == preview.unit_id)
        {
            let world = layout.hex_to_world_pos(selected.hex);
            let top_height = map
                .as_ref()
                .map(|map| {
                    map.tiles
                        .iter()
                        .filter(|tile| tile.hex == selected.hex && tile.layer == selected.layer)
                        .map(|tile| tile.bottom + tile.height)
                        .fold(0.0, f32::max)
                })
                .unwrap_or(0.0);
            let scale = entity.get_float("scale", 1.0).log_fallback(worker_tx);
            let z = top_height;
            let rotation = entity.get_float("rotation_y", 0.0).log_fallback(worker_tx);
            let position = Vec3::new(world.x, z, world.y);
            let material = entity
                .get_material(&state.materials)
                .log_fallback(worker_tx);
            set_material(
...
fn draw_preview_cell(
    ...
    hex_mesh: &Mesh,
    map: &Option<pystral_core::domain::HexMap>,
    cell: &pystral_core::log::AvailableMove,
    color: [f32; 3],
    height: f32,
) {
    let world = layout.hex_to_world_pos(cell.hex);
    let top_height = map
        .as_ref()
        .map(|map| {
            map.tiles
                .iter()
                .filter(|tile| tile.hex == cell.hex && tile.layer == cell.layer)
                .map(|tile| tile.bottom + tile.height)
                .fold(0.0, f32::max)
        })
        .unwrap_or(0.0);
    let model = Mat4::from_translation(Vec3::new(world.x, top_height + 0.02, world.y))
        * Mat4::from_scale(Vec3::new(
            layout.scale.x * 0.92,
            height,
            layout.scale.y * 0.92,
        ));
    set_model_matrix(&ctx.gl, &ctx.uniforms, &model);
    set_material(&ctx.gl, &ctx.uniforms, color, 0.2, 0.0, 0.0);
    ctx.gl.uniform1i(ctx.uniforms.use_tex.as_ref(), 0);
    hex_mesh.draw(&ctx.gl, ctx.attribs.pos, ctx.attribs.norm, ctx.attribs.uv);
}
