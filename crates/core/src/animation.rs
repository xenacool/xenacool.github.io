...
impl ActiveFSM {
    ...
    pub fn transition_to(&mut self, target_state: String, now_ms: f64) {
        if self.current_state_name == target_state { return; }
...
pub fn interpolate(start: &PropertyValue, end: &PropertyValue, t: f32) -> PropertyValue {
    match (start, end) {
        (PropertyValue::Float(s), PropertyValue::Float(e)) => {
            PropertyValue::Float(s + (e - s) * t)
        }
        (PropertyValue::Vec3(s), PropertyValue::Vec3(e)) => {
            PropertyValue::Vec3(glam::Vec3::from_array(s.to_array()) + (glam::Vec3::from_array(e.to_array()) - glam::Vec3::from_array(s.to_array())) * t)
        }
