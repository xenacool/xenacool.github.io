...
impl ActiveFSM {
    ...
    /// Presentation consumers may use the same clock as property tracks
    /// without reimplementing FSM transition timing.
    pub fn elapsed_ms(&self, now_ms: f64) -> f32 {
        (now_ms - self.state_start_time_ms).max(0.0) as f32
    }
