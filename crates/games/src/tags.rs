use crate::jobs::DefinitionId;
use pystral_core::ui_log::{LogCommand, Logger};
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::hash::Hash;

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct TagId(pub DefinitionId);
...
impl TagBag {
    pub fn count(&self, tag: TagId) -> u8 {
        ...
        if let Some(def) = defs.defs.get(&tag) {
            ...
            *current = (*current).saturating_add(n).min(def.max_stacks);
            ...
            logger.apply_command(LogCommand::Error(format!(
                "Attempted to emit undefined tag: {:?}",
                tag
            )));
        }
    }
