...
impl RPGProgram {
    ...
    pub fn add_op(&mut self, op: RPGBytecode) -> Result<(), String> {
        self.ops.push(op);
        self.validate()
    }
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for op in &self.ops {
            match op {
                ...
                RPGBytecode::HealCaster { amount } if *amount < 0 => {
                    return Err("Heal amount must not be negative".to_string());
                }
                RPGBytecode::HealCasterFromActualDamage { denominator, .. }
                    if *denominator == 0 =>
                {
                    return Err("Actual-damage heal denominator must be positive".to_string());
                }
                RPGBytecode::SpawnOwnedUnit {
                    job,
                    maximum_active,
                } if job.trim().is_empty() || *maximum_active == 0 => {
                    return Err(
                        "Spawned job must be named and maximum active must be positive".to_string(),
                    );
                }
                _ => {}
            }
