    ...
    #[test]
    fn reachable_cells_rejects_destinations_with_a_solid_ceiling() {
        let mut config = SkirmishConfig::new(42);
        config
            .add_unit(1, 1, "Caveman", GridCell::new(Hex::new(0, 0), 0))
            .expect("default unit config");
        let mut state = config.build_state().expect("default skirmish config");
        let destination = GridCell::new(Hex::new(1, 0), 0);
        let ceiling = GridCell::new(destination.hex, 1);
        state.grid.bounds.max_layer = 1;
        state.grid.set_tile(ceiling, TileType::Rock).unwrap();
        assert!(!state.grid.has_unit_clearance(destination));
        let reachable = reachable_cells(&state, AgentId(1)).unwrap();
        assert!(!reachable.contains_key(&destination));
    }
