    ...
    #[test]
    fn movement_and_reaction_execute_from_authored_fields_without_name_dispatch() {
        let mut scenario = SkirmishConfig::new(42);
        scenario
            .add_unit(1, 1, "Mage", GridCell::new(hexx::Hex::ZERO, 0))
            .unwrap();
        scenario
            .add_unit(2, 2, "Caveman", GridCell::new(hexx::Hex::new(2, 0), 0))
            .unwrap();
        let mut state = scenario.build_state().unwrap();

        let movement_id = state.agents[&AgentId(1)].movement_ability;
        let movement = state.movement_registry.get_mut(&movement_id).unwrap();
        movement.name = "modded movement".into();
        movement.mana_gain = 7;
        state.agents.get_mut(&AgentId(1)).unwrap().mana = 0;
        let moved = execute_composite(
            &state,
            AgentId(1),
        ...
        .unwrap();
        assert_eq!(moved.agents[&AgentId(1)].mana, 7);

        let reaction_id = state.agents[&AgentId(1)].reaction_abilities[0];
        let reaction = state.reaction_registry.get_mut(&reaction_id).unwrap();
        reaction.name = "modded reaction".into();
        reaction.ap_cost = 1;
        reaction.mana_cost = 3;
        reaction.target_damage = 9;
        reaction.self_heal = 5;
        let reacting = state.agents.get_mut(&AgentId(1)).unwrap();
        reacting.action_points = 4;
        reacting.mana = 10;
        reacting.health -= 10;
        state.reaction_queue = vec![(AgentId(1), reaction_id, AgentId(2))];
        let target_health = state.agents[&AgentId(2)].health;
        let reacted = execute_composite(
            &state,
            AgentId(1),
        ...
        assert_eq!(
            (
                reacted.agents[&AgentId(1)].action_points,
                reacted.agents[&AgentId(1)].mana,
                reacted.agents[&AgentId(1)].health,
                reacted.agents[&AgentId(2)].health,
            ),
            (
                3,
                7,
                state.agents[&AgentId(1)].health + 5,
                target_health - 9,
            )
        );
    }
