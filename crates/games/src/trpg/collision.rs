...
impl CollisionGeometry {
    /// Canonical vertical clearance for every unit: two hex heights.
    pub const UNIT_HEIGHT_HEXES: f32 = 2.0;

    pub fn unit_height(self) -> f32 {
        Self::UNIT_HEIGHT_HEXES * self.hex_width()
    }
...
impl CollisionWorld {
    ...
    pub fn from_state(
        ...
        for (&agent, unit) in units.iter().filter(|(_, unit)| unit.health > 0) {
