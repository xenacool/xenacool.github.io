...
pub fn legal_ability_targets(
    ...
    let mut targets = state
        .agents
        .iter()
        .filter(|(target, unit)| match ability.target_rule {
            crate::AbilityTargetRule::SelfUnit => **target == agent && unit.health > 0,
            crate::AbilityTargetRule::EnemyUnit => {
                **target != agent && unit.health > 0 && unit.team_id != attacker.team_id
            }
            crate::AbilityTargetRule::OwnedSummon => {
                unit.health > 0
                    && state
                        .summons
                        .get(target)
            ...
            crate::AbilityTargetRule::AreaCell | crate::AbilityTargetRule::EmptyCell => false,
        })
        .filter_map(|(target, _)| {
...
pub(crate) fn ability_target_is_legal_with_world(
    ...
    match ability.target_rule {
        crate::AbilityTargetRule::SelfUnit if target != agent => return false,
        crate::AbilityTargetRule::SelfUnit => return true,
        crate::AbilityTargetRule::EnemyUnit
        ...
        crate::AbilityTargetRule::AreaCell | crate::AbilityTargetRule::EmptyCell => return false,
        _ => {}
    }

    if let Some(required) = ability.facing_rule
        && attacker
            .facing
            .relation_to(attacker.position.hex, defender.position.hex)
            != Some(required)
    {
        return false;
    }
    ...
    match ability.delivery {
        ...
        AbilityDelivery::SelfTarget => true,
    }
}
    ...
    fn execute(
        ...
        if let Some(unit) = ctx.state_diff.get_agent_mut(self.agent) {
            if let Some(def) = reaction_def {
                if unit.action_points < i32::from(def.ap_cost)
                    || unit.mana < i32::from(def.mana_cost)
                {
                    return None;
                }
                unit.action_points -= i32::from(def.ap_cost);
                unit.mana -= i32::from(def.mana_cost);
                unit.apply_healing(def.self_heal);
                if let Some(target_unit) = ctx.state_diff.get_agent_mut(self.target) {
                    target_unit.apply_damage(def.target_damage);
                }
            }
        }
    ...
    fn execute(
        ...
        if let Some(unit) = ctx.state_diff.get_agent_mut(self.agent) {
            ...
            if let Some(prog) = prog {
                ...
                unit.mana = unit
                    .mana
                    .saturating_add(prog.mana_gain)
                    .min(unit.derived_stats.mana_max);
            }
        }
        None
    }
