...
pub fn calculate_damage(
    ...
    defender: &UnitState,
    ability: &AbilityDef,
    passives: &std::collections::HashMap<crate::PassiveId, crate::PassiveDef>,
    modifier_card: ModifierCard,
    logger: &mut Logger,
) -> i32 {
    let attacker_stats = attacker.stats_with_passives(passives);
    let defender_stats = defender.stats_with_passives(passives);
    ...
    let damage = modified_vector
        .component_sub_clamped(defense_vector)
        .sum()
        .floor()
        .max(0.0) as i32;
    let (numerator, denominator) = attacker
        .passive_abilities
        .iter()
        .filter_map(|id| passives.get(id))
        .fold((1_i64, 1_i64), |(numerator, denominator), passive| {
            (
                numerator.saturating_mul(i64::from(passive.damage_multiplier.0)),
                denominator.saturating_mul(i64::from(passive.damage_multiplier.1)),
            )
        });
    let damage = i32::try_from(i64::from(damage).saturating_mul(numerator) / denominator.max(1))
        .unwrap_or(i32::MAX);
    let (relation_numerator, relation_denominator) = attacker
        .facing
        .relation_to(attacker.position.hex, defender.position.hex)
        .map(|relation| ability.facing_damage_multiplier(relation))
        .unwrap_or((1, 1));
    i32::try_from(
        i64::from(damage).saturating_mul(i64::from(relation_numerator))
            / i64::from(relation_denominator.max(1)),
    )
    .unwrap_or(i32::MAX)
}
