...
fn test_damage_calculation() {
    ...
    let ability = AbilityDef {
        ...
        target_rule: AbilityTargetRule::EnemyUnit,
...
#[test]
fn passive_effects_apply_from_authored_fields_without_name_dispatch() {
    let mut attacker = setup_unit(1, 100, 10);
    let defender = setup_unit(2, 100, 10);
    let passive_id = PassiveId(900);
    attacker.passive_abilities = vec![passive_id];
    let passives = HashMap::from([(
        passive_id,
        PassiveDef {
            id: passive_id,
            name: "modded passive".into(),
            stat_modifiers: vec![(DerivedStat::Constitution, 4)],
    ...
    let ability = AbilityDef {
        ...
        emit_tags: vec![],
        consume_tags: vec![],
        scaling: HashMap::from([("CON".into(), 1.0)]),
        range: 1,
        delivery: AbilityDelivery::Melee,
        target_rule: AbilityTargetRule::EnemyUnit,
        projectile_profile: None,
...
fn timed_modifiers_stack_independently_or_replace_by_policy() {
    ...
    .unwrap();
    assert_eq!(unit.stats_with_passives(&HashMap::new()).armor_class, 8);
    unit.advance_owner_turn();
    assert_eq!(unit.stats_with_passives(&HashMap::new()).armor_class, 4);
    unit.add_timed_modifier(TimedModifier {
        stat: DerivedStat::ArmorClass,
        amount: 9,
        remaining_turns: 1,
    ...
    .unwrap();
    assert_eq!(unit.stats_with_passives(&HashMap::new()).armor_class, 10);
}
