...
impl Runtime {
    pub(super) fn refresh_available_actions(&self, unit_id: u64) -> RuntimeResponse {
        if self.continuation != (RuntimeContinuation::AwaitPlayerDecision { unit_id }) {
            return RuntimeResponse::Error(
                "Available-action refresh requested outside the matching player boundary"
                    .to_string(),
            );
        }
        self.pg_rpg_sim
            .as_ref()
            .and_then(|simulation| simulation.get_available_actions(unit_id as i64))
            .map(|actions| RuntimeResponse::AvailableActionsRefreshed { actions })
            .unwrap_or_else(|| RuntimeResponse::Error(format!("Unknown player unit {unit_id}")))
    }
    ...
    pub(super) fn commit_ability_request(
        ...
        if let Some(sim) = self.pg_rpg_sim.as_ref() {
            for event in Self::summon_lifecycle_events(&simulation_before_request, sim) {
                history.push_and_apply(event);
            }
        }
        if let Some(sim) = self.pg_rpg_sim.as_ref() {
        ...
        match response {
            RuntimeResponse::ActionCommitted {
                ...
                self.continuation = RuntimeContinuation::AwaitAnimationAck {
                    ...
                    unit_id: *unit_id,
                    ends_turn: action == "face" || (action == "wait" && !npc_boundary),
                    npc: npc_boundary,
    ...
    pub(super) fn commit_move_request(
        ...
        if let Some(unit) = self
            .pg_rpg_sim
            .as_ref()
            .and_then(|sim| sim.state.agents.get(&validated.agent))
        {
            history.push_and_apply(Event::UpdateProperty {
                id: validated.agent.0 as u64,
    ...
    pub(super) fn commit_facing_request(
        &mut self,
        request_id: u64,
        unit_id: u64,
        facing: &str,
    ) -> RuntimeResponse {
        if let Err(message) = self.ensure_facing_boundary(unit_id) {
            return RuntimeResponse::Error(message);
        }
        let Some(sim) = self.pg_rpg_sim.as_mut() else {
            return RuntimeResponse::Error("Simulation not started".into());
        };
        if let Err(reason) = sim.commit_facing(unit_id, facing) {
    ...
    pub(super) fn commit_end_turn_request(
        &mut self,
        request_id: u64,
        unit_id: u64,
    ) -> RuntimeResponse {
        let mut response = self.commit_wait_request(request_id, unit_id);
        if let RuntimeResponse::ActionCommitted { action, .. } = &mut response {
    ...
    pub(super) fn ensure_decision_boundary(&self, unit_id: u64) -> Result<(), String> {
        match self.continuation {
            ...
            RuntimeContinuation::AwaitPlayerFacing { unit_id: expected } if expected == unit_id => {
                Ok(())
            }
            RuntimeContinuation::AwaitPlayerFacing { unit_id: expected } => Err(format!(
                "Only facing may be submitted for unit {expected} after End Turn"
            )),
    ...
    pub(super) fn ensure_facing_boundary(&self, unit_id: u64) -> Result<(), String> {
        match self.continuation {
            RuntimeContinuation::AwaitPlayerFacing { unit_id: expected } if expected == unit_id => {
                Ok(())
            }
            RuntimeContinuation::AwaitPlayerFacing { unit_id: expected } => Err(format!(
                "Facing for unit {unit_id} is stale; unit {expected} owns the facing boundary"
            )),
            RuntimeContinuation::AwaitAnimationAck { .. } => {
                Err("Facing submitted before End Turn animation acknowledgment".into())
            }
            _ => Err("Facing is only available after End Turn".into()),
        }
    }
}
