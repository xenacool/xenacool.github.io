...
use pystral_core::log::{AvailableActions, AvailableMove, Event, GameOutcome};
...
pub enum RuntimeResponse {
    ...
    /// A slice budget exhausted without reaching a boundary. The request
    /// resolves here (see `spec/SimulationBridge.tla`'s `RunSlice`) and the
    /// simulation is still advancing, so the worker keeps auto-stepping from
    /// this slice until the next boundary or the game completes.
    PgRpgSimulationSlice {
        work_units: u32,
    ...
    pub fn process_request(&mut self, request: RuntimeRequest) -> (RuntimeResponse, Vec<String>) {
        ...
        let response = match request {
            ...
            RuntimeRequest::CommitEndTurn {
                request_id,
                unit_id,
            } => self.commit_end_turn_request(request_id, unit_id),
...
#[cfg(test)]
mod necromancer_tests;
#[cfg(test)]
mod npc_protocol_tests;
#[cfg(test)]
mod projectile_lifecycle_tests;
#[cfg(test)]
