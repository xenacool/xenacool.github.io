...
fn facing_is_only_available_after_end_turn_and_costs_no_ap() {
    let mut runtime = runtime_with_unit();
    let opponent =
        runtime.pg_rpg_sim.as_ref().unwrap().state.agents[&npc_engine_core::AgentId(1)].clone();
    let mut opponent = opponent;
    opponent.team_id = 2;
    opponent.position.hex = hexx::Hex::new(2, 0);
    runtime
        .pg_rpg_sim
        .as_mut()
        .unwrap()
        .state
        .agents
        .insert(npc_engine_core::AgentId(2), opponent);
    runtime
        .pg_rpg_sim
        .as_mut()
        .unwrap()
        ...
        .insert(
            ...
            pystral_games::UnitController::Npc,
        );
    let before =
        runtime.pg_rpg_sim.as_ref().unwrap().state.agents[&npc_engine_core::AgentId(1)].clone();
    assert!(matches!(
        runtime
            .process_request(RuntimeRequest::CommitFacing {
                request_id: 13,
                unit_id: 1,
                facing: "northwest".into(),
            })
            .0,
        RuntimeResponse::Error(_)
    ));
    let response = runtime
        .process_request(RuntimeRequest::CommitEndTurn {
            request_id: 12,
            unit_id: 1,
        })
        .0;
    let end_turn_barrier = match response {
        RuntimeResponse::ActionCommitted {
            action, barrier_id, ..
        } => {
            assert_eq!(action, "end_turn");
            barrier_id
        }
        other => panic!("expected end-turn commit, got {other:?}"),
    };
    assert!(matches!(
        runtime
            .process_request(RuntimeRequest::AcknowledgeAnimation {
                barrier_id: end_turn_barrier
            })
            .0,
        RuntimeResponse::Continuation(RuntimeContinuation::AwaitPlayerFacing { unit_id: 1 })
    ));
    let response = runtime
        .process_request(RuntimeRequest::CommitFacing {
            request_id: 13,
    ...
    let barrier_id = match response {
        RuntimeResponse::ActionCommitted {
            ...
            assert_eq!(action, "face");
            assert!(
                matches!(history.log[0], Event::UpdateProperty { id: 1, ref property, value: pystral_core::log::PropertyValue::String(ref facing) } if property == "facing" && facing == "northwest")
            );
            barrier_id
        }
        other => panic!("expected facing commit, got {other:?}"),
    };
    let after = &runtime.pg_rpg_sim.as_ref().unwrap().state.agents[&npc_engine_core::AgentId(1)];
    assert_eq!(after.facing, pystral_games::Facing::Northwest);
    assert_eq!(after.action_points, before.action_points);
    assert!(matches!(
        runtime
            .process_request(RuntimeRequest::AcknowledgeAnimation { barrier_id })
            .0,
