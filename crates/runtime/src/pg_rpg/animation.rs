use pystral_compiler::ik::{IkRequest, IkSystem, Vec3 as IkVec3};
use pystral_compiler::physics::{TrajectoryRequest, TrajectorySystem};
    ...
    for i in 0..=num_steps {
        ...
        if let Ok(response) = ik_system.solve(&request) {
            for (joint, pos) in &response.joints {
                tracks
                    .entry(format!("{}_x", joint))
                    .or_default()
                    .push(Keyframe {
                        time_ms: t_ms,
                        value: PropertyValue::Float(pos.x),
                    });
                tracks
                    .entry(format!("{}_y", joint))
                    .or_default()
                    .push(Keyframe {
                        time_ms: t_ms,
                        value: PropertyValue::Float(pos.y),
                    });
                tracks
                    .entry(format!("{}_z", joint))
                    .or_default()
                    .push(Keyframe {
                        time_ms: t_ms,
                        value: PropertyValue::Float(pos.z),
                    });
            }
        }
    }
    ...
    match trajectory_system.solve(&request, map) {
        Ok(response) => {
            ...
            for (i, pos) in response.trajectory.iter().enumerate() {
                ...
                x_keyframes.push(Keyframe {
                    time_ms: t,
                    value: PropertyValue::Float(pos.x),
                });
                y_keyframes.push(Keyframe {
                    time_ms: t,
                    value: PropertyValue::Float(pos.z),
                }); // 3D Z is Y in our engine
                z_keyframes.push(Keyframe {
                    time_ms: t,
                    value: PropertyValue::Float(pos.y),
                }); // 3D Y is Z in our engine

                let pitch = response.rotations[i];
                pitch_keyframes.push(Keyframe {
                    time_ms: t,
                    value: PropertyValue::Float(pitch),
                }); // Positive pitch to tilt up
                yaw_keyframes.push(Keyframe {
                    time_ms: t,
                    value: PropertyValue::Float(response.yaw),
                });
            }

            tracks.push(PropertyTrack {
                property: "world_x".into(),
                keyframes: x_keyframes,
                loop_behavior: LoopBehavior::Loop,
            });
            tracks.push(PropertyTrack {
                property: "world_y".into(),
            ...
            tracks.push(PropertyTrack {
                property: "rotation_z".into(),
                keyframes: pitch_keyframes,
                loop_behavior: LoopBehavior::Loop,
            });
            tracks.push(PropertyTrack {
                property: "rotation_y".into(),
                keyframes: yaw_keyframes,
                loop_behavior: LoopBehavior::Loop,
            });
        }
        Err(_e) => {
