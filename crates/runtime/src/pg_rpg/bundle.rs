...
impl ScenarioBundle {
    ...
    pub fn root_rhai_at(&self, entrypoint: &str) -> Result<String, String> {
        ...
        // Root-relative includes resolve against the entrypoint's top-level
        // directory (the rhai scripts root, e.g. "scripts").
        let root = entrypoint.split('/').next().unwrap_or("").to_string();
    ...
    fn append_rhai(
        ...
        for line in contents.lines() {
            if let Some(include) = line.trim().strip_prefix("// @include ") {
                ...
                let child = resolve_relative(path, include, root)?;
                self.append_rhai(&child, files, root, visiting, loaded, output)?;
            } else {
...
fn resolve_relative(parent: &str, child: &str, root: &str) -> Result<String, String> {
    // Includes are relative to the including file's directory. A leading '/'
    // is root-relative (the rhai scripts root) so files in different
    // directories can share includes without traversal segments.
    let path = if let Some(rooted) = child.strip_prefix('/') {
        if root.is_empty() {
            rooted.to_string()
        } else {
            format!("{root}/{rooted}")
...
mod tests {
    ...
    #[test]
    fn resolves_root_relative_includes_from_nested_files() {
        let bundle = ScenarioBundle {
            rhai_files: vec![
                text(
                    "scripts/fixtures/casualty.rhai",
                    "// @include \"/scenarios/skirmish.rhai\"\nlet hp = 1;",
                ),
                text("scripts/scenarios/skirmish.rhai", "let skirmish = 2;"),
            ],
            ..Default::default()
        };
        assert_eq!(
            bundle
                .root_rhai_at("scripts/fixtures/casualty.rhai")
                .unwrap(),
            "let skirmish = 2;\nlet hp = 1;\n"
        );
    }
