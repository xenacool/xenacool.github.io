...
use pystral_games::{AbilityId, AgentId, legal_ability_targets};
...
impl TacticalSimulation {
    pub fn ability_targets(
        ...
        let cost = ability.resolve_cost(&attacker.turn_tags);
        if !cost.can_pay(attacker) {
            return (
                Vec::new(),
                Some(format!("Insufficient resources for {}", ability.name)),
            );
        }
        let mut targets = match ability.target_rule {
            pystral_games::AbilityTargetRule::AreaCell => self
                .state
                .grid
                .tiles
                .keys()
                ...
                .collect(),
            pystral_games::AbilityTargetRule::EmptyCell => self
                .state
                .grid
                .tiles
                .keys()
                .filter(|cell| {
                    attacker.position.layer == cell.layer
                        && attacker.position.hex.distance_to(cell.hex) <= i32::from(ability.range)
                        && !self
                            .state
                            .agents
                            .values()
                            .any(|unit| unit.health > 0 && unit.position == **cell)
                })
                .map(|cell| AbilityTarget {
                    kind: AbilityTargetKind::Cell,
                    hex: cell.hex,
                ...
                .collect(),
            pystral_games::AbilityTargetRule::SelfUnit => vec![AbilityTarget {
            ...
            pystral_games::AbilityTargetRule::EnemyUnit
            | pystral_games::AbilityTargetRule::OwnedSummon => {
                legal_ability_targets(&self.state, agent, AbilityId(ability_id as u32))
                    .into_iter()
                    .filter_map(|target| {
