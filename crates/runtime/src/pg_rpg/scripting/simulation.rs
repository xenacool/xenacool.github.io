...
use pystral_games::{
    ...
    ScriptReactionDef, ScriptTagDef, SkirmishConfig, UnitController,
};
    ...
    engine
        ...
        .register_fn(
            ...
            |config: &mut SkirmishConfig,
             ...
             mana: i64|
             -> Result<(), Box<rhai::EvalAltResult>> {
                config.set_unit_mana(id, mana).map_err(runtime_error)
            },
        )
        .register_fn(
            "set_unit_controller",
            |config: &mut SkirmishConfig,
             ...
             -> Result<(), Box<rhai::EvalAltResult>> {
                let controller = match controller {
                    "Player" => UnitController::Player,
                    "Npc" => UnitController::Npc,
                    "WaitOnly" => UnitController::WaitOnly,
                    value => {
                        return Err(runtime_error(format!("Unknown unit controller: {value}")));
                    }
                };
                config
                    .set_unit_controller(id, controller)
                    .map_err(runtime_error)
            },
        )
        .register_fn(
            "set_unit_summon_controller",
            |config: &mut SkirmishConfig,
             id: i64,
             controller: &str|
             -> Result<(), Box<rhai::EvalAltResult>> {
                let controller = match controller {
                    ...
                    "WaitOnly" => UnitController::WaitOnly,
                    value => {
                        return Err(runtime_error(format!("Unknown summon controller: {value}")));
                    }
                };
                config
                    .set_unit_summon_controller(id, controller)
                    .map_err(runtime_error)
            },
        );
    register_projectile_config(engine);
    engine.register_fn(
...
fn register_projectile_config(engine: &mut Engine) {
    engine
        .register_fn(
...
fn register_simulation_runtime(engine: &mut Engine) {
    ...
    // Keep the long-standing Hex-only helper above for scripts that only need
    // horizontal placement. Presentation code that must preserve tactical
    // elevation uses the full cell instead of silently assuming layer zero.
    engine.register_fn("get_agent_cell", |sim: &mut TacticalSimulation, id: i64| {
        sim.get_agent_position(id)
    });
    engine.register_fn(
        "get_agent_health",
        |sim: &mut TacticalSimulation, id: i64| sim.get_agent_health(id) as i64,
    );
    engine.register_fn("get_agent_kind", |sim: &mut TacticalSimulation, id: i64| {
        sim.get_agent_kind(id)
    });
    engine.register_fn(
        "set_agent_health",
    ...
    engine.register_fn(
        "set_agent_position",
        |sim: &mut TacticalSimulation, id: i64, position: GridCell| {
            sim.set_agent_position(id, position).map_err(runtime_error)
        },
    );
    engine.register_fn(
        "set_agent_controller",
        |sim: &mut TacticalSimulation, id: i64, controller: &str| {
            let controller = match controller {
                "Player" => UnitController::Player,
                "Npc" => UnitController::Npc,
                "WaitOnly" => UnitController::WaitOnly,
                value => return Err(runtime_error(format!("Unknown controller: {value}"))),
            };
            sim.set_agent_controller(id, controller)
                .map_err(runtime_error)
        },
    );
    engine.register_fn(
        "set_agent_summon_controller",
        |sim: &mut TacticalSimulation, id: i64, controller: &str| {
            let controller = match controller {
                "Npc" => UnitController::Npc,
                "WaitOnly" => UnitController::WaitOnly,
            ...
            sim.set_agent_summon_controller(id, controller)
                .map_err(runtime_error)
        },
    );
