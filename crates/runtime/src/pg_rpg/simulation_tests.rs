...
#[test]
fn npc_gets_one_zero_cost_facing_action_after_primary_action() {
    let mut scenario = SkirmishConfig::new(42);
    scenario
        .add_unit(1, 1, "Caveman", GridCell::new(hexx::Hex::ZERO, 0))
        .unwrap();
    ...
    let primary = simulation.request_npc_decision(AgentId(2)).unwrap();
    assert!(!matches!(primary, TacticalDisplayAction::Face { .. }));
    simulation.apply_npc_action(AgentId(2), primary).unwrap();
    let ap_after_primary = simulation.state.agents[&AgentId(2)].action_points;
    let facing = simulation.request_npc_decision(AgentId(2)).unwrap();
    assert!(matches!(facing, TacticalDisplayAction::Face { .. }));
    simulation.apply_npc_action(AgentId(2), facing).unwrap();
    assert_eq!(
        simulation.state.agents[&AgentId(2)].action_points,
        ap_after_primary
    );
    assert!(
        simulation
            .request_npc_decision(AgentId(2))
            .is_some_and(|action| !matches!(action, TacticalDisplayAction::Face { .. }))
    );
}
