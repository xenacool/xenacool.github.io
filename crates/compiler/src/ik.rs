    ...
    pub fn solve(&self, request: IkRequest) -> Result<IkResponse, String> {
        ...
        for var_name in &rig.variable_names {
            if let Some(pos) = request.initial_guesses.get(var_name) {
                ...
                let bias_x = if var_name.starts_with('l') { -0.3 } else if var_name.starts_with('r') { 0.3 } else { 0.0 };
        ...
        let result = rig.solver.solve(values).map_err(|e| format!("IK Solver error: {:?}", e))?;
        
        let mut joints = HashMap::new();
        for var_name in &rig.variable_names {
            let x = *result.values.get(&format!("{}_x", var_name)).unwrap_or(&0.0);
            let y = *result.values.get(&format!("{}_y", var_name)).unwrap_or(&0.0);
            let z = *result.values.get(&format!("{}_z", var_name)).unwrap_or(&0.0);
            joints.insert(var_name.clone(), Vec3 { x: x as f32, y: y as f32, z: z as f32 });
        }
...
pub fn length_eq(ax: &Exp, ay: &Exp, az: &Exp, bx: &Exp, by: &Exp, bz: &Exp, length: f64) -> Exp {
    ...
    let dist_sq = Exp::add(Exp::add(Exp::power(dx, 2.0), Exp::power(dy, 2.0)), Exp::power(dz, 2.0));
