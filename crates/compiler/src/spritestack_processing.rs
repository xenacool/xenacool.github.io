...
pub fn quantize_rgb_channels(rgba: &mut [u8], bits: u8) -> usize {
    assert!((1..=8).contains(&bits), "RGB quantization needs 1..=8 bits");
    let levels = (1u16 << bits) - 1;
    let mut changed = 0;
    for pixel in rgba.as_chunks_mut::<4>().0 {
        for channel in &mut pixel[..3] {
            let quantized = (((u16::from(*channel) * levels) + 127) / 255) * 255 / levels;
            let quantized = quantized as u8;
...
mod tests {
    ...
    #[test]
    fn quantizes_rgb_deterministically_without_changing_alpha() {
        let mut rgba = image(2, 1, &[[13, 127, 254, 17], [255, 0, 128, 231]]);
        let changed = quantize_rgb_channels(&mut rgba, 5);
        assert_eq!(changed, 4);
        assert_eq!(rgba[3], 17);
