...
test.describe('Debug Panels BDD', () => {
  ...
  test('Sync debug panels when scrubbing history', async ({ page }) => {
    ...
    await test.step('When I scrub the history slider', async () => {
      ...
      await page.waitForFunction(() => {
        const entry = document.getElementById('history-entry-10');
        return entry?.classList.contains('current');
      }, { timeout: 10000 });
    });
    ...
    await test.step('When I click a history entry in the viewer', async () => {
        await page.locator('#history-entry-5').evaluate((entry) => entry.click());
        await expect(page.locator('#history-slider')).toHaveValue('5');
    });

    await test.step('Then the history slider and value should update', async () => {
        await page.waitForFunction(() => (
          document.getElementById('history-slider')?.value === '5'
          && document.getElementById('slider-value')?.innerText === '5'
        ), { timeout: 10000 });
    });
  });
});
