...
async function castFireball(page) {
  ...
  const ability = page.locator('[data-menu-key^="ability:"]').filter({ hasText: 'Fireball' });
  ...
  const abilityKey = await ability.getAttribute('data-menu-key');
...
async function loadScenario(page, scenario) {
  await loadWithFixture(page, scenario);
