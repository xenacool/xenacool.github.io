...
async function main() {
  ...
  for (const file of glbFiles) {
    ...
    try {
      const bakeInfo = await page.evaluate(async ({ modelName, file, idleTime }) => {
        ...
        return {
          layerCount: layerStore.layers.length,
      ...
      fs.writeFileSync(
        path.join(modelOutputDir, 'bake-metadata.json'),
        JSON.stringify({ version: 1, model: modelName, ...bakeInfo }, null, 2),
      );
